...
const axios = require('axios'); // For Meta API
...
// Initialize Twilio and Meta API clients
...
// API function to send messages via Twilio
...
// API function to send messages via Meta
async function sendViaMeta(to, message) {
    ...
    try {
        ...
        const response = await axios.post(url, {
            ...
            to: to.replace('whatsapp:', ''), // Meta doesn't need the 'whatsapp:' prefix
            type: "text",
            text: {
                body: message
            }
        ...
        return { success: true, provider: 'Meta', result: response.data };
    } catch (error) {
...
// Message sender with Failover Logic
async function sendWhatsAppMessage(to, message) {
    // Primary API: Meta
    try {
        console.log(`Attempting to send via Meta...`);
        return await sendViaMeta(to, message);
    } catch (metaError) {
        console.error(`Meta failed. Attempting to send via Twilio...`);
        // Secondary API: Twilio
        try {
            return await sendViaTwilio(to, message);
        } catch (twilioError) {
            console.error(`Twilio also failed.`);
            throw new Error('Both Meta and Twilio failed to send the message.');
        }
    }
}
...
app.post('/meta-webhook', async (req, res) => {
    ...
    // Check if the webhook event is a new message
    if (body.entry && body.entry[0].changes && body.entry[0].changes[0].value.messages) {
        const from = body.entry[0].changes[0].value.messages[0].from;
        const incomingMsg = body.entry[0].changes[0].value.messages[0].text.body;
        ...
        // This handles other webhook events like status updates
        res.status(200).send('EVENT_RECEIVED');
    }
});
